...
AKECS_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ECS_IOCTL_GET_OPEN_STATUS:
		...
		if (ret < 0) {
			//dev_err(&akm->i2c->dev,"Get Open returns error (%d).", ret);
		}
		break;
	case ECS_IOCTL_GET_CLOSE_STATUS:
		...
		if (ret < 0) {
			//dev_err(&akm->i2c->dev, "Get Close returns error (%d).", ret);
		}
		break;
	case ECS_IOCTL_GET_DELAY:
...
// added by yangze 20120901 start
static struct device_attribute akm8963_attributes_client[] = {

	__ATTR(enable_device, 0660, akm8963_enable_mag_show, akm8963_enable_mag_store),
	__ATTR(pollrate_ms,  0660, akm8963_delay_mag_show,  akm8963_delay_mag_store),
	__ATTR_NULL,
};
//added by yangze 20120901 end
...
static int akm_parse_dt(struct device *dev,  struct akm8963_platform_data *pdata)
{
        ...
        if (rc && (rc != -EINVAL)) {
                dev_err(dev, "Unable to read layout\n");
                return rc;
        } else {
                pdata->layout = temp_val;
        }

        rc = of_property_read_u32(np, "akm,outbit", &temp_val);
        if (rc && (rc != -EINVAL)) {
                dev_err(dev, "Unable to read outbit\n");
                return rc;
        } else {
                pdata->outbit = temp_val;
        }

	temp_val = of_get_named_gpio(np, "akm,gpio-rstn", 0);
	if (temp_val < 0) {
		dev_err(dev, "Unable to read gpio_rstn\n");
	} else {
		pdata->gpio_RST = temp_val;
	}

	temp_val = of_get_named_gpio(np, "akm,gpio-drdy", 0);
	if (gpio_is_valid(temp_val)) {
		rc = gpio_request(temp_val, "akm_drdy");
		if (rc) {
...
int akm8963_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/***** Set layout information *****/
        if (client->dev.of_node) {
                ...
                err = akm_parse_dt(&client->dev, pdata);
                if (err) {
                        dev_err(&client->dev,
                ...
                printk("akm8963 not use device tree\n");
	...
	s_akm->i2c = client;
        ...
        if (err < 0) {
                dev_err(&s_akm->i2c->dev, "power init failed! err=%d", err);
                goto exit2;
        }

        if (gpio_is_valid(s_akm->rstn)) {
                err = gpio_request(s_akm->rstn, "akm_rstn");
                if (err) {
	...
	for (i=0; i<AKM_NUM_SENSORS; i++){
		s_akm->delay[i] = -1;
		// added by yangze 20120903 start
		if(1 == i){
			s_akm->delay[1] = 200;
		}
		// added by yangze 20120903 end
	}
...
static ssize_t akm_info_read_proc(char *page, char **start, off_t off,
				int count, int *eof, void *data)
{
	return sprintf(page, "0x%x\n", chipid);
}

static struct proc_dir_entry *akm_info_proc_file;
static int __init create_akm_info_proc_file(void)
{
        if(0xff == chipid) {
                printk("not create akm8963 proc file due to unconnected\n");
                return 0;
        }  
  akm_info_proc_file = create_proc_entry("driver/compass", 0644, NULL);
  //printk("goes to create_acc_info_proc_file\n");
  if (akm_info_proc_file) {
			akm_info_proc_file->read_proc = akm_info_read_proc;
   } else{
